...
##############################################################
# https://github.com/lsuperman/DataAugmentForObjectDetection
# 包括:
#	 1. 裁剪(需改变bbox)
#	 2. 平移(需改变bbox)
#	 3. 改变亮度
...
class DataAugmentForObjectDetection():
	...
	def _rotate_img_bbox(self, img, bboxes, angle=5, scale=1.):
		...
		输入:
			img:图像array,(h,w,c)
			bboxes:该图像包含的所有boundingboxs,一个list,每个元素为[x_min, y_min, x_max, y_max, name]
			angle:旋转角度
			scale:默认1
		输出:
			rot_img:旋转后的图像array
	...
	def _filp_pic_bboxes(self, img, bboxes):
		'''
			...
			输入:
				img:图像array
				bboxes:该图像包含的所有boundingboxs,一个list,每个元素为[xmin, ymin, xmax, ymax, name]
			输出:
	...
	# 镜像
	def _filp_pic_bboxes_8points(self, img, bboxes):
		'''
			参考:https://blog.csdn.net/jningwei/article/details/78753607
			平移后的图片要包含所有的框
			输入:
				img:图像array
				bboxes:该图像包含的所有boundingboxs,一个list,每个元素为[x1, y1, x2, y2, x3, y3, x4, y4],要确保是数值
			输出:
				flip_img:平移后的图像array
				flip_bboxes:平移后的bounding box的坐标list
		'''
		...
		for box in bboxes:
			...
			flip_bboxes.append([w-x1, y1, w-x2, y2, w-x3, y3, w-x4, y4])
	...
	def dataAugment(self, img, bboxes):
		...
		#img, bboxes = self._filp_pic_bboxes_8points(img, bboxes)
		return img, bboxes
...
if __name__ == '__main__':
	...
	need_aug_num = 5                        # 每张原始图片扩充的图片数量			  

	prex = 'argu1_'                         # 扩充图片的名字的前缀
	source_pic_root_path = 'JPEGImages/'    # 待扩充图片的路径
	source_xml_root_path = 'xml/'           # 待扩充图片的标签文件路径
	
	argument_pic_save_path = 'argu_images/' # 保存扩充图片的路径
	argument_xml_save_path = 'argu_xml/'    # 保存扩充图片对应的标签文件路径
    
    ##############################################################################
    ##############################################################################
    ##############################################################################
	
	if not os.path.exists(argument_pic_save_path):
		os.mkdir(argument_pic_save_path)
	
	if not os.path.exists(argument_xml_save_path):
		os.mkdir(argument_xml_save_path)
	...
	for parent, _, files in os.walk(source_pic_root_path):
		for file in files:
			...
			print('argument file {} '.format(file))
			shutil.copyfile(pic_path, os.path.join(argument_pic_save_path, file))
			shutil.copyfile(xml_path, os.path.join(argument_xml_save_path, file[:-4]+'.xml'))
			all_image_num += 1
			...
			while cnt < need_aug_num:
                ...
                # img, coords -- [[x_min,y_min,x_max,y_max,name]]
                # auged_bboxes -- [[x_min,y_min,x_max,y_max,name]]
				...
				for bbox in auged_bboxes:
					...
					boxes.append(box)	   
				# print(boxes)		 
				new_imgname = prex + file.split('.')[0] + '_' + str(cnt) + '.' + file.split('.')[1]
				#print(new_imgname)
				cv2.imwrite(os.path.join(argument_pic_save_path, new_imgname), auged_img)
				generate_xml(new_imgname, boxes, auged_img.shape, argument_xml_save_path)
				
				all_image_num += 1
	
	print('all generate images : ', all_image_num)
	print('done...')
